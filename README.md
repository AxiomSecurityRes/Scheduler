...
| 8 | **캘린더 뷰** | 리스트 ↔ **월간 캘린더** 토글, 마감일을 점(Dot)으로 표시, 날짜 클릭 시 해당일 일정 |
| 9 | **문서 첨부** | 이미지 외 **PDF·HWPX·PPTX·DOCX 등 문서 업로드/다운로드**(상세 페이지 다운로드 링크) |
| 10 | **태그 & FAQ** | `#수행평가양식` 등 **태그 검색/필터**, 관리자 고정 **FAQ 탭**(검색·태그) |
| 11 | **NEIS 위젯** | **오늘의 급식 + 우리 반 시간표**(나이스 오픈API, 학교명 자동 코드 탐색) 대시보드 위젯 |
...
│       ├── dashboard.js       # 대시보드 + 추가/수정 폼 + 태그/캘린더 토글
│       ├── calendar.js        # 경량 월간 캘린더 뷰
│       ├── detail.js          # 상세 + 이미지 슬라이더 + 문서 다운로드 + Q&A
│       ├── faq.js             # FAQ 탭(검색·태그·관리자 편집)
│       ├── neis.js            # NEIS 급식·시간표 위젯
